...
void on_button1_button_press_event(GtkButton *object, gpointer user_data)
{
  char *str1;
  entry_value = gtk_entry_get_text(GTK_ENTRY((GtkWidget *) user_data));
...
void dof()
{
  int sockid, newsockid,i,getfile,ack,msg,msg_2,c,len;
    ...
    FILE *fp; 
    char in_buf[MAXSIZE];
    if(argc != 2) {printf("error: usage : ftp filename\n"); exit(0);}
    no_writen = 0;
    num_blks = 0;
    num_last_blk = 0;
    len = strlen(argv[1]);
